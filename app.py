...
NEW_SIGNAL_TEMPLATE = """
<h1>Ny signal</h1>
<form method="post" onsubmit="return validateSignalType()">
  <label>Datum *<input type="date" name="date" value="{{ today }}" required></label>
  <label>Person *<input type="text" name="person" required></label>
  <label>Organisation<input type="text" name="organization"></label>
  <label>Signal-typ *
    <select name="signal_type_select">
      <option value="">-- välj --</option>
      {% for t in signal_types %}<option value="{{ t }}">{{ t }}</option>{% endfor %}
    </select>
  </label>
  <label>...eller skriv egen signal-typ (åsidosätter valet ovan)<input type="text" name="signal_type_other"></label>
  <label>Roll/möjlighet (valfritt)<input type="text" name="role_opportunity"></label>
  <label>Kanal
  ...
  <label>...eller skriv egen kanal<input type="text" name="channel_other"></label>
  <label>Vad hände? *<textarea name="note" required></textarea></label>
  <label>Vad lärde jag mig?<textarea name="learning"></textarea></label>
  <label>Vilket problem/behov hörde jag?<textarea name="problem_heard"></textarea></label>
  <label>Vad skapade intresse för min bakgrund?<textarea name="interest_signal"></textarea></label>
  <label>Problem-taggar (kommaseparerat)<input type="text" name="problem_tags"></label>
  <label>Roll-taggar (kommaseparerat)<input type="text" name="role_tags"></label>
  ...
  <fieldset>
    ...
    <label>Befintlig hypotes
      <select name="hypothesis_id">
        <option value="">-- ingen --</option>
        {% for h in hypotheses %}<option value="{{ h['id'] }}">{{ h['statement'] }}</option>{% endfor %}
      </select>
    </label>
    <label>Eller skriv en ny hypotes<input type="text" name="new_hypothesis"></label>
    <label>Relation
      <select name="relation">
        <option value="supports">Stödjer</option>
        <option value="contradicts">Motsäger</option>
      </select>
    </label>
  </fieldset>

  <label>Nästa steg (valfritt)<input type="text" name="next_action"></label>

  <button type="submit" class="btn-primary">Spara signal</button>
</form>
<script>
...
def new_signal():
    ...
    if request.method == "POST":
        form = request.form
        signal_type = (form.get("signal_type_other") or "").strip() or form.get(
            "signal_type_select", ""
        ).strip()
        channel = (form.get("channel_other") or "").strip() or (
            form.get("channel_select") or ""
        ).strip() or None
        cur = db.execute(
            """INSERT INTO signals
        ...
        for text in parse_tag_list(form.get("problem_tags", "")):
            tag_id = get_or_create_tag(db, text, "problem")
            db.execute(
                "INSERT OR IGNORE INTO signal_tags (signal_id, tag_id) VALUES (?, ?)",
                (signal_id, tag_id),
            )
        for text in parse_tag_list(form.get("role_tags", "")):
            tag_id = get_or_create_tag(db, text, "role")
            db.execute(
                "INSERT OR IGNORE INTO signal_tags (signal_id, tag_id) VALUES (?, ?)",
                (signal_id, tag_id),
            )
    ...
    return render_template_string(
        page("Ny signal", NEW_SIGNAL_TEMPLATE),
